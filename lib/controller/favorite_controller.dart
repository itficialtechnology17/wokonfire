...
import 'package:wokonfire/model/model_favorite.dart';
...
class FavoriteController extends GetxController {
  ...
  var arrOfFavorite = List<ModelFavorite>().obs;
  HomeController _homeController = Get.find();

  var isLoading = false.obs;
  var isAddToFavoriteLoading = false.obs;
  ...
  void _apiGetFavorite() async {
    Request request = Request(url: urlGetFavorite, body: {
      'type': "API",
      'c_id': userId.toString(),
      'r_id': restaurantId.toString(),
    });
    request.post().then((value) {
      final responseData = json.decode(value.body);

      arrOfFavorite.value = (responseData['favorite'] as List)
          .map((data) => ModelFavorite.fromJson(data))
          .toList();
                    ...
                    InkWell(
                      ...
                      child: Obx(() => Visibility(
                            ...
                            child: Container(
                              ...
                              height: Get.height * 0.06,
                              color: Colors.red,
                              child: !isAddToFavoriteLoading.value
                                  ? Center(
                                      child: Text(
  ...
  void _apiManageFavoriteMaster(String favoriteTitle, String actionType,
    ...
    Request request = Request(url: urlManageFavoriteMaster, body: {
      ...
      'f_m_id': index == null ? "" : arrOfFavoriteTitle[index].fMId.toString(),
  ...
  void apiManageFavorite(BuildContext context, String actionType, int foodId,
    ...
    request.post().then((value) {
      if (actionType != "DELETE") {
        ...
        _homeController
            .arrOfDashboardTitle[index].foodItems[nIndex].isFavorite = 0;
      }
      ...
      if (responseData["status_code"] == 1) {
        if (actionType == "ADD") {
          ...
          _homeController.arrOfDashboardTitle[index].foodItems[nIndex]
              .isFavorite = responseData["id"];
        }
      } else {
        print("Add,Update Or Delete Failed");
      }
